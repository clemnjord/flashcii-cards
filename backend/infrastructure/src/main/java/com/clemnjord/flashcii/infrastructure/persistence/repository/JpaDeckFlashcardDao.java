...
public interface JpaDeckFlashcardDao extends JpaRepository<DeckFlashcardEntity, DeckFlashcardEntityId> {
    ...
    @Query("SELECT df.deck FROM DeckFlashcardEntity df WHERE df.flashcard.id.flashcardId = :flashcardId AND"
            + " df.flashcard.id.ownerId = :ownerId")
    ...
    boolean existsByDeck_UuidAndFlashcard_Id_FlashcardIdAndFlashcard_Id_OwnerId(
            UUID deckId, UUID flashcardId, UUID ownerId);
