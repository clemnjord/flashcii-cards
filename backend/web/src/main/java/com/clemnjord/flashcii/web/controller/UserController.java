...
public class UserController {
    ...
    public FlashcardDto.FlashcardResponse createFlashcard(
            @PathVariable String userId, @Valid @RequestBody FlashcardDto.FlashcardRequest flashcardRequest) {
        Flashcard flashcard = createFlashcardUseCase.execute(new CreateFlashcardCommand(
                UserId.from(userId), new Question(flashcardRequest.question()), new Answer(flashcardRequest.answer())));
