...
public class GlobalExceptionHandler {
    ...
    public ApiErrorResponse handleUserAlreadyExists(UserAlreadyExistsException ex) {
        return new ApiErrorResponse("USER_ALREADY_EXISTS", ex.getMessage(), HttpStatus.CONFLICT.value(), Instant.now());
    }

    @ExceptionHandler(InvalidDeckException.class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public ApiErrorResponse handleInvalidDeck(InvalidDeckException ex) {
        return new ApiErrorResponse("INVALID_DECK", ex.getMessage(), HttpStatus.BAD_REQUEST.value(), Instant.now());
    }
    ...
    public record ValidationErrorResponse(
            String errorCode, String message, Map<String, String> fieldErrors, int status, Instant timestamp) {}
}
